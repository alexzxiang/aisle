...
**Anthropic Claude (Haiku 4.5 and Sonnet 5).** Structured JSON output is exactly what a safety minded app needs, and Claude read shelves, signs and openings better than anything else we tried. One unsupported schema keyword (minItems) silently broke every vision call for hours, and five seconds per frame is two metres for a walking user, so we would love loud schema validation and a mode tuned for short structured answers on small images.

**NVIDIA Nemotron on NIM.** Fast and dependable for turning speech into structured tasks, and the free credits made it an easy choice. Rate limits arrived without warning, so a quota view in the response headers would have let us plan instead of adding a second key and a template fallback.

**NVIDIA Brev.** Generous credits and a setup simple enough to prepare an Open Images fine tuning job in an hour. The weekend's hours went to the search logic instead, which is on us, not the platform.

**ElevenLabs.** The voice is why Aisle Be There feels like a companion rather than a screen reader, and Scribe's key terms kept item names accurate. We wish for streaming recognition that returns a one word "yes" without waiting for silence.

**Expo SDK 57 with a development build and a local Swift module.** Bridging our own Swift perception module through Expo worked better than expected. The SDK has changed enough that every coding assistant reached for last year's APIs, and native changes need a slow clean rebuild.

**Apple ARKit, Core ML and Vision.** A plain iPhone 16 ran pose tracking, two detectors, a depth model, full frame OCR and hand tracking at once, and Vision's OCR reads overhead signs better than any cloud call on a downscaled frame. The limits were hardware truths: no metric depth without LiDAR and no ultra wide camera inside an ARKit session.

**Ultralytics YOLO and Open Images.** Exporting to Core ML with built in NMS was one command, which is why we could run two detectors at all. The stock Open Images nano is spread thin across six hundred classes, so home fixtures like doors and countertops need a narrowed model.

**GitHub and GitHub Actions.** Five people on one repository for twenty four hours with a small CI kept us honest. The 100 MB file limit rejected our demo video at the last minute; a nudge toward LFS before the push would help hackathon teams.

**Claude Code and Codex as teammates.** Both wrote large parts of this codebase overnight and were excellent at tracing real failures back to code and writing replay tests from a tester's exact words. Two agents will happily rewrite the same forty lines in opposite directions, so a human has to read the diff and make them argue with the field trace rather than with each other.
