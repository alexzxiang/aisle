...
func trackerChecks() {
  ...
  var cartEvidence = CartEvidenceFilter()
  let cartCandidate = RawDetection(cls: .cart, box: box(cx: 0.5, cy: 0.6, w: 0.4, h: 0.4), score: 0.9)
  check("cart: isolated frame withheld", cartEvidence.apply([cartCandidate], at: 1).isEmpty)
  check("cart: repeated confident frame accepted", cartEvidence.apply([cartCandidate], at: 1.2).count == 1)
  check("cart: stale sighting withheld", cartEvidence.apply([cartCandidate], at: 3).isEmpty)
