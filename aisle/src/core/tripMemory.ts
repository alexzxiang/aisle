...
export function createTripMemory(now: () => number = Date.now) {
  ...
  const invalidate = () => { ready = false; stable = 0; current = null; lastPortal = null; walkedSincePortal = 0; history = []; poseNodes.clear(); generation++; };
  ...
  return {
    ...
    defer(item: string, id = current?.id, ms = 120000) {
      if (id) deferred.set(`${id}:${norm(item)}`, now() + ms);
    ...
    snapshot() { return { ready, epoch, places: places.map(n => ({ ...n, items: [...n.items] })), edges: edges.map(e => ({ ...e })), portals: portals.map(p => ({ ...p })), aisles: aisles.map(a => ({ ...a })), points: [...voxels.values()].map(p => ({ ...p })), evidence: evidence.map(e => ({ ...e })) }; },
  };
}
