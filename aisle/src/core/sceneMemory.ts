...
export function classForWords(words: string): DetectionClass | null {
  ...
  const table: ReadonlyArray<[RegExp, DetectionClass]> = [
    ...
    [/\b(headphones|earphones|earbuds?|airpods?|air pods?|headset)\b/, 'headphones'], [/\b(hats?|caps?|beanies?|cowboy hats?|sun hats?)\b/, 'hat'],
    [/\b(watch|wristwatch)\b/, 'watch'], [/\b(wheelchair)\b/, 'wheelchair'],
