...
|---|---|---|
| `coco-yolo-nano.mlpackage`, `coco-yolo-nano-416.mlpackage` | AGPL-3.0 (Ultralytics YOLO11 / YOLOv8 pretrained weights) | Hackathon prototype, source published; weights are downloaded and exported per machine (`npm run models:venv && npm run models:coco`), not vendored. Trained on COCO (CC BY 4.0 annotations). |
...
| `depth-anything-v2-small.mlpackage` | Apache-2.0 — verified 2026-09-18 on the model card. Base / Large are CC BY-NC-4.0 and are not to be used. | Source: Hugging Face `apple/coreml-depth-anything-v2-small`, file `DepthAnythingV2SmallF16.mlpackage`, commit `cfef6f6f2a70783dedc0bfae40cecbc2052285d3`. Downloaded per machine, not vendored (`npm run models:depth`). |
