...
export const VISION_PROMPTS: Readonly<Record<VisionQuestion, string>> = Object.freeze({
  ...
  task_step: [
    ...
    'Recognize common personal objects when their shape and visible details support the identity: hats and caps, wallets, AirPods or wireless-earbud cases, keys, glasses, watches, remotes, phones, chargers and bags. A generic headphones box does not verify AirPods, and a handbag does not verify a wallet. For small white earbud cases, wallets and keys, require a detailed close view or distinctive features; otherwise keep target.box null and request a closer angle.',
    'Recognize grocery foods including milk, juice, pizza, packaged chicken, beef, pork, steak, bacon, ham, sausage, turkey, fish and seafood. Packaging color alone is insufficient. Use readable labels, distinctive exposed contents or characteristic packaging together; never reinterpret a live animal detection as packaged meat. Keep target.box null when the package identity is uncertain.',
    ...
    'Fill search from this image: quality, current view, readable current-area sign, confidently visible foods, and up to four landmarks with boxes. When userText has an active objective to leave a room or aisle, list visible openings first; omit nearby tables and containers from landmarks. A doorway may surround the camera at its frame edges when the user stands in it: visible floor continuing into a different space can establish an open passage even if no door leaf appears. In a store, a gap between shelving with a cross corridor can be kind doorway or aisle_end; give it boundary cross_aisle. Landmarks are the ways on from this spot, and a blind person cannot find them without you: the end or opening of the aisle where it meets a corridor (kind aisle_end); a doorway, door, gap between shelves, corridor mouth or exit (kind doorway); a sign, banner or display naming a department in the distance (kind section, with its section); a counter, table, shelf unit or appliance worth walking to (surface / appliance). Name each in one or two stable words ("aisle end", "left doorway", "produce sign") and keep the name between frames; when unsure whether something is an opening, list it with confidence 0.5 to 0.7 rather than leaving it out. A sign across the store is a destination landmark, not the current-area sign. A category guess is not an aisle number or a direction. Return unknown when evidence is weak.',
