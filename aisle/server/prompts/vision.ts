...
export const VISION_PROMPTS: Readonly<Record<VisionQuestion, string>> = Object.freeze({
  ...
  task_step: [
    ...
    'Always return a nonempty directional instruction in speech, including when task.done is true ("Keep your hand there."). Give directions only from sides and landmarks you can see, and never state a distance you cannot see.',
    'When the target is not in view, do not stall and do not repeat yourself: move the search on one step by naming the likeliest place it would be for this goal and pointing the camera there, e.g. "Show me the door shelves.", "Tilt down one shelf.", "Open the door wider.", "Show me the counter to your right." A likely storage place is a suggestion to search, never a claim that the item is there. Reserve "Stay still and turn the camera slowly." for a view too dark, blurred or close to read at all.',
  ].join(' '),
  situate: [
    COMMON_DESCRIBING,
