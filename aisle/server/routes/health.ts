 ...
 * re-runs everything (pre-demo readiness). `?budgetMs=` shortens each fresh
 * probe's own timeout (doctor's fast path: a cold report otherwise waits up to
 * `HEALTH_TIMEOUT_MS` per check). `GET /api/health/warm` re-fires every warm-up
 * pair and returns their status (a stop-the-line check 30 minutes out).
 */
...
export function createHealthRouter(src: DepsSource): Router {
  ...
  router.get('/', async (req, res) => {
    ...
    const t0 = Date.now();
    const budget = Number(req.query.budgetMs);
    const timeoutMs = Number.isFinite(budget) ? Math.min(MAX_BUDGET_MS, Math.max(MIN_BUDGET_MS, budget)) : undefined;
    const report = await deps.health.report({ force: req.query.force === '1', timeoutMs });
